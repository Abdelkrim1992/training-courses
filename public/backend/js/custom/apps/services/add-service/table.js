    ...
    var initDeleteRow = () => {
        rowSelector.querySelectorAll('[data-kt-customer-table-filter="delete_row"]').forEach(button => {
            button.addEventListener("click", function(event) {
                ...
                const serviceName = row.querySelectorAll("td")[1].innerText;
                const serviceId = row.getAttribute('data-service-id'); // assuming you have set data-client-id attribute
    ...
    return {
        init: function() {
            ...
            rowSelector.querySelectorAll("tbody tr").forEach(row => {
                ...
                const formattedDate = moment(cells[3].innerHTML, "DD MMM YYYY, LT").format();
